...
.application.properties（可以覆盖默认值）
[source%nowrap,properties]
----
include::{resourcesdir}/result.application.properties[]
----

指定你对外展示的描述：

.messages.properties（可以覆盖默认值）
[source%nowrap,properties]
----
Result.success=请求成功
Result.error=请求失败
Result.errors=共有{0}个错误
Result.argument_type_mismatch=参数"{0}"的值"{1}"无法被转换成"{2}"类型
----

[[validation_code]]
.ValidationMessages.properties（可以覆盖默认值）
[source%nowrap,properties]
----
...
抛出异常::
在代码中抛出异常：``throw new ResultException(resultBuilder.build("6666", new Object[]{id, User.class}));``，
...
[source%nowrap,javascript]
----
include::{testsourcedir}/com/github/peacetrue/result/user.json[]
----
...
[source%nowrap,javascript]
----
include::{testsourcedir}/com/github/peacetrue/result/error.json[]
----
...
例如：想要指定成功编码为**200**，可使用``peacetrue.result.codes.success=success200``。
