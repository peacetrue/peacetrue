 ...
 * @author xiayx
 */
public class ArgCreatorIdProvider implements CreatorIdProvider {

    private Logger logger = LoggerFactory.getLogger(getClass());

    @Override
    public Object getCreatorId(AfterMethodBasedEvaluationContext context) {
        Object id = Arrays.stream(context.getArguments())
                .filter(arg -> arg instanceof OperatorCapable).findFirst()
                .map(arg -> ((OperatorCapable) arg).getOperatorId()).orElse(null);
